...
There is a much more serious problem that essentially prevents the use of these AI tools for analyzing entire articles. Unfortunately, in its current form, GPT has a limited context length, which causes Auto-GPT to simply refuse to start the analysis, and chatbots lose the beginning of the conversation:
